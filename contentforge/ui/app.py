...
Professional desktop GUI with collapsible network menus and PNG icons
"""
...
class EmojiPicker(ctk.CTkToplevel):
    ...
    def __init__(self, parent, callback):
        ...
        container = ctk.CTkScrollableFrame(self, fg_color=COLORS["bg_card"], corner_radius=10)
        container.pack(fill="both", expand=True, padx=10, pady=10)
        
        for category_name, emojis in EMOJI_CATEGORIES.items():
            cat_label = ctk.CTkLabel(container, text=category_name, font=FONTS["heading"], text_color=COLORS["text_primary"])
            ...
            for i, emoji in enumerate(emojis):
                btn = ctk.CTkButton(
                    emoji_frame, text=emoji, width=35, height=35,
                    font=("Segoe UI Emoji", 16), fg_color=COLORS["bg_input"],
                    hover_color=COLORS["bg_hover"], command=lambda e=emoji: self._select(e)
                )
...
class ContentForgeApp(ctk.CTk):
    """Main application window with collapsible network menus and PNG icons."""
    ...
    def _create_sidebar(self):
        self.sidebar = ctk.CTkScrollableFrame(self, width=260, corner_radius=0, fg_color=COLORS["bg_card"])
        ...
        ctk.CTkLabel(title_frame, text="🚀", font=("Segoe UI Emoji", 36)).pack()
        ctk.CTkLabel(title_frame, text="ContentForge", font=FONTS["title"], text_color=COLORS["text_primary"]).pack()
        ctk.CTkLabel(title_frame, text="AI Content Generator", font=FONTS["small"], text_color=COLORS["text_secondary"]).pack()
        
        ctk.CTkFrame(self.sidebar, height=2, fg_color=COLORS["bg_hover"]).pack(fill="x", padx=15, pady=15)
        ctk.CTkLabel(self.sidebar, text="REDES SOCIALES", font=FONTS["small"], text_color=COLORS["text_muted"]).pack(anchor="w", padx=15, pady=(0, 10))
        
        # Network sections with icons
    ...
    def _create_network_section(self, network: Dict):
        ...
        # Get PNG icon if available, fallback to emoji
        icon = get_network_icon(network_id)
        
        if icon:
            header_btn = ctk.CTkButton(
                container, text=f"  {network['name']}  ▼", image=icon,
                compound="left", font=FONTS["body"], height=42, anchor="w",
            ...
            header_btn = ctk.CTkButton(
                container, text=f"{network['icon']}  {network['name']}  ▼",
                font=FONTS["body"], height=42, anchor="w",
                fg_color="transparent", hover_color=COLORS["bg_hover"],
                text_color=COLORS["text_primary"],
                command=lambda n=network_id: self._toggle_network(n)
            )
        ...
        for type_id, type_info in CONTENT_TYPES.items():
            ...
            if content_icon:
                type_btn = ctk.CTkButton(
                    types_frame, text=f"  {type_info['name']}", image=content_icon,
                    compound="left", font=FONTS["small"], height=34, anchor="w",
                ...
                type_btn = ctk.CTkButton(
                    types_frame, text=f"  {type_info['icon']} {type_info['name']}",
                    font=FONTS["small"], height=34, anchor="w",
                    fg_color="transparent", hover_color=COLORS["accent_primary"],
                    text_color=COLORS["text_secondary"],
                    command=lambda n=network_id, t=type_id: self._select_content_type(n, t)
                )
    ...
    def _toggle_network(self, network_id: str):
        if self.expanded_network and self.expanded_network != network_id:
            ...
            icon = get_network_icon(self.expanded_network)
            if icon:
    ...
    def _select_content_type(self, network_id: str, content_type: str):
        ...
        if network and type_info:
            self.header_label.configure(text=f"{network['icon']} {network['name']} → {type_info['icon']} {type_info['name']}")
    ...
    def _create_main_area(self):
        ...
        self.header_label = ctk.CTkLabel(
            main_frame, text="👈 Selecciona una red social y tipo de contenido",
        ...
        ctk.CTkLabel(input_frame, text="📝 Tema o idea principal:", font=FONTS["heading"], text_color=COLORS["text_primary"]).grid(row=0, column=0, padx=15, pady=(15, 5), sticky="w")
        
        self.topic_entry = ctk.CTkTextbox(
            input_frame, height=80, font=FONTS["body"],
        ...
        self.generate_btn = ctk.CTkButton(
            main_frame, text="✨ GENERAR CONTENIDO", font=FONTS["heading"], height=50,
        ...
        ctk.CTkLabel(header_row, text="📄 CONTENIDO GENERADO (editable):", font=FONTS["heading"], text_color=COLORS["text_primary"]).grid(row=0, column=0, sticky="w")
        
        actions = ctk.CTkFrame(header_row, fg_color="transparent")
        actions.grid(row=0, column=1, sticky="e")
        
        self.emoji_btn = ctk.CTkButton(actions, text="😀 Emojis", width=90, height=30, font=("Segoe UI Emoji", 11), fg_color=COLORS["bg_input"], hover_color=COLORS["bg_hover"], command=self._open_emoji_picker)
        self.emoji_btn.pack(side="left", padx=3)
        
        self.copy_btn = ctk.CTkButton(actions, text="📋 Copiar", width=80, height=30, font=FONTS["small"], fg_color=COLORS["bg_input"], hover_color=COLORS["bg_hover"], command=self._copy_content)
        self.copy_btn.pack(side="left", padx=3)
        
        self.regen_btn = ctk.CTkButton(actions, text="🔄 Regenerar", width=95, height=30, font=FONTS["small"], fg_color=COLORS["bg_input"], hover_color=COLORS["bg_hover"], command=self._regenerate_content)
        self.regen_btn.pack(side="left", padx=3)
        
        self.output_text = ctk.CTkTextbox(
            output_frame, font=("Segoe UI Emoji", 12),
    ...
    def _generate_content(self):
        ...
        def generate():
            ...
            self.after(0, lambda: self._on_complete(formatted))
    ...
    def _format_for_network(self, content: str) -> str:
        ...
        header = f"{'═' * 50}\n{network['icon']} {network['name']} | {type_info['icon']} {type_info['name']}\n{'═' * 50}\n\n"
